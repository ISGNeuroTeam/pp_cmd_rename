...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"rename_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f rename-*.tar.gz
	echo Create archive \"rename-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../rename-$(VERSION)-$(BRANCH).tar.gz rename
...
rename.tar.gz: build
	cd make_build; tar czf ../rename.tar.gz rename && rm -rf ../make_build
